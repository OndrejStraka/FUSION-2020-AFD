...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% NODE 1
% Node 1 - Model 1
node(1).M(1).A = [0.76 0.01];
node(1).M(1).B = 0.12;
node(1).M(1).G = sqrt(0.003);
node(1).M(1).C = 0.9;
node(1).M(1).H = 0.01;
% Node 1 - Model 2
node(1).M(2).A = [0.86 0.03];
node(1).M(2).B = 0.14;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% NODE 2
% Node 2 - Model 1
node(2).M(1).A = [0.02 0.87];
node(2).M(1).B = 0.13;
node(2).M(1).G = sqrt(0.002);
node(2).M(1).C = 0.9;
node(2).M(1).H = 0.01;
% Node 2 - Model 2
node(2).M(2).A = [0.01 0.775];
node(2).M(2).B = 0.15;
...
if design
  ...
  parfor iSubsys = 1:2 % VALUE ITERATION
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ON-LINE PART
% Initialize the predictive estimates for the first time step and each node
k = 1;
for iNode = 1:nNode
...
tic
for imc = 1:nmc
  fprintf('H:<strong>%s</strong>,GlobalDecision:<strong>%s</strong>,Feedback:<strong>%s</strong>\t MC simulation #:<strong>%i</strong>\n',hierarchy,mat2str(centralDecision),mat2str(returnPmuToLocalnodes),imc)
  k = 1;
    ...
    switch hierarchy
      ...
      case 'distributed'
        ...
        switch mergeMethod
          ...
          case 'GPB2'
            estimate(:,k) = lssmmkff_GPB2(y(:,k),estimate(:,k),node,nodeStates);
          otherwise
            error('unknown merge method')
        end
      otherwise
...
fprintf(fid,'%s\t %s\t design:%s\t on-line:%s\t approxMethod:%s\t centralDecision:%s\t returnPmu:%s\n',datetime('now'),hierarchy,designMergeMethod,mergeMethod,localP,mat2str(centralDecision),mat2str(returnPmuToLocalnodes));
fprintf(fid,'MC:\t\t %u\n',nmc);
fprintf(fid,'J:\t\t %f\n',mean(J));
